<x-layout :title="$title ?? null">
    <april:sidebar-layout class="items-start px-3">
        <april:sidebar collapsible="none"
            class="hidden h-[calc(100vh-16px)] w-full bg-background shrink-0 lg:sticky lg:top-0 lg:flex lg:w-fit lg:min-w-[270px] max-h-[calc(100vh-16px)]">
            <slot:content x-persist="menu" class="beautify-scrollbar" wire:scroll>
                <x-menu />
            </slot:content>
        </april:sidebar>

        <april:sidebar-inset class="relative docs-layout lg:mx-auto px-2 lg:max-w-3xl py-6 lg:gap-10 lg:py-8">
            <div class="space-y-2">
                @isset($title)
                <h1 class="scroll-m-20 text-4xl font-bold tracking-tight">
                    {{$title}}
                </h1>
                @endisset
                @isset($description)
                <p class="text-lg text-muted-foreground">
                    {{$description}}
                </p>
                @endisset
            </div>
            {!!$slot!!}
        </april:sidebar-inset>
    </april:sidebar-layout>
</x-layout>
